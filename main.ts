...
if ( !defaultBrowser || !config[defaultBrowser] ) die(`Error: No default specified or default refers to unknown browser`);
...
const regexify = (s:string): RegExp|string => { const r = regexRegex.exec(s); return(r ? new RegExp(r[1]) : s); }
const matched = new Set();
for ( const browser in config ) {
    if ( browser == 'default') continue;
    VERBOSE && console.log(`Testing for ${browser}`);
    if ( ! config[browser].command ) die(`Error: Browser has no command param: '${browser}`);
...
if ( matched.size == 0 ) {
    ...
    if ( matched.size > 1 ) {
        ...
        console.warn(`Warning: Multiple browsers matched, using first (${browserList})`);
    }
